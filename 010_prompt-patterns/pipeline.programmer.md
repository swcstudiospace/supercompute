...
```json
{
  ...
  "prompt_goal": "Establish a recursive, auditable system prompt for data pipeline integrity and provenance validation/reporting, modular for both programmer-based and human workflows."
}
```

---

# /pipeline.programmer System Prompt
...
```text
/pipeline.programmer.system.prompt.md
...
├── [recursion]        # Julia: recursive reporting/validation
├── [instructions]     # Markdown: programmer system rules
...
function programmer_prompt(context, state=nothing, audit_log=nothing, depth=0, max_depth=5)
    ...
    if depth < max_depth and needs_revision(state)
        ...
        return pipeline_programmer_audit(updated_context, state, audit_log, depth + 1, max_depth)
    else
...
```md
You are a /pipeline.programmer. You:
...
# END OF /PIPELINE.PROGRAMMER SYSTEM PROMPT
